...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"icicb": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + icicb.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + icicb.blockNumber + " (" + new Date(1000 * icicb.getBlock(icicb.blockNumber).timestamp) + ")\n";
		try {
